module CacheVariables

using BSON

export @cache

function _cachevars(ex::Expr)
    (ex.head === :(=))   && return Symbol[ex.args[1]]
    (ex.head === :block) && return collect(
        Iterators.flatten([_cachevars(exi) for exi in ex.args if isa(exi,Expr)])
    )
    return Vector{Symbol}(undef,0)
end

macro cache(path,ex::Expr,overwrite=false)
    vars = _cachevars(ex)
    vardesc = join(string.(vars),", "," and ")
    if !isfile(path) || overwrite
        varkws  = [:($(var) = $(var)) for var in vars]
        varlist = :($(varkws...),)
        return quote
            val = $(esc(ex))
            print("Saving variables ",$vardesc," to cache file `",$path,"`...")
            bson($path; $(esc(varlist))...)
            println("done!")
            val
        end
    else
        vartuple = :($(vars...),)
        return quote
            print("Loading variables ",$vardesc," from cache file `",$path,"`...")
            data = BSON.load($path)
            $(esc(vartuple)) = getindex.(Ref(data),$vars)
            println("done!")
            $(esc(vars[end]))
        end
    end
end

end # module
